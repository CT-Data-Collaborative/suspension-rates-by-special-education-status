...
##################################################################
#
# Processing Script for Suspension-Rates-by-Special-Education-Status
...
#Rename SpEd column
complete_susp_rates_long$`Special Education Status` <- "Special Education"
...
test <- complete_susp_rates_long[,c("District", "Year", "Special Education Status", "Measure Type")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_susp_rates_long,
  file.path(path_to_top_level, "data", "suspension_rate_sped_2017.csv"),
  sep = ",",
  row.names = F
)
